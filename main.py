...
def add_to_bd(m, res = False):
    bot.send_message(m.chat.id, 'Вас приветствует база данных студентов Geek Brains!\n'\
                                'Введите нужную команду для работы с БД:\n'\
                                '/1 - вывести все записи\n'\
                                '/2 - добавить запись\n'\
                                '/3 - найти запись\n'\
                                '/4 - изменить запись\n'\
                                '/5 - удалить запись\n'\
                                '/6 - сортировка базы данных по полю')

# _______________________________________________________________________


@bot.message_handler(commands = ["1"]) # вывод всех записей БД
def print_all(message):
    print_base = printing.print_base(base)
    bot.send_message(message.chat.id, f'База данных студентов:\n{print_base}')
